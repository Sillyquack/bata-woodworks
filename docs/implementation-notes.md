...
The React/Vite application is a public client. It receives only a Supabase project URL, publishable key and non-secret launch configuration. All authority-bearing operations run in Supabase Edge Functions with the server role. `anon` and `authenticated` have no business-table grants or application RPC access; the private buckets have no browser-role Storage policies. RLS remains enabled as defense in depth.
...
| Private offer link | Display one token-selected offer and initiate its exact payment | The raw token is stored only in the URL fragment/email and browser session return mapping; the database stores SHA-256 only and the payment provider never receives it |
...
- Provider return URLs contain only the server-generated provider reference and are display-only. The client keeps an ephemeral session mapping to restore the private offer route. Only an authenticated webhook event plus a provider snapshot/capture for the exact amount can produce `PAID`.
...
- After payment, the customer and management receive payment confirmations that do not claim manufacture has started. Management explicitly advances `PAID → PRODUCTION`; that transition sends the customer a separate production-start notice. Progression then continues `PRODUCTION → READY → DELIVERED`.
...
- Public functions use explicit origin allowlists, narrow methods, generic customer-facing errors and no ambient table access. Production CORS accepts only `https://batawoodworks.no`.
...
- `LEGAL_TRADING_ENABLED=true` is required for offer issuance/resend and payment initiation. It stays false until registered identity and owner-approved terms exist.
- `VIPPS_LIVE_ENABLED=true` is a separate production interlock. Mock payments require `APP_ENV=local|test`, `PAYMENT_PROVIDER=mock` and `ALLOW_MOCK_PAYMENTS=true`; a production environment rejects mock checkout even if the other switches are misconfigured.
...
- Transactional email uses the verified Bata identities, accessible HTML plus plain text, deliberate Reply-To routing and stable provider idempotency keys. Notification records survive provider failure; the manager queue surfaces failures and explicit offer resend uses a new, rotatable token.
- Logs use internal/provider record IDs and sanitized error summaries. They do not include raw offer tokens, customer bodies, authorization headers, provider bodies or credentials.
...
- `supabase/migrations/20260822132139_restrict_data_api_to_edge_functions.sql` — removes browser-role data privileges and obsolete browser manager policies; adds hot-path partial indexes
...
- Legal pages are noindex launch drafts, not fabricated legal advice. Transactional operations fail closed until the ENK/organisation number, VAT status, business contact details and Norwegian owner-approved legal text exist.
- A generic failed-notification retry endpoint is intentionally out of scope. The manager queue and production runbook provide manual recovery without repeating the underlying state change or payment.
